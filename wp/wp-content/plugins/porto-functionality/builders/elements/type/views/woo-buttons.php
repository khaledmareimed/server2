	...
	if ( 'cart' == $atts['link_source'] ) {
		...
		if ( apply_filters( 'porto_product_loop_show_price', true ) ) {
			...
			echo apply_filters(
				...
				sprintf(
					'<%s title="%s" href="%s" aria-describedby="woocommerce_loop_add_to_cart_link_describedby_%s" data-quantity="%s" class="' . esc_attr( apply_filters( 'porto_elements_wrap_css_class', $btn_classes, $atts, 'porto-tb/porto-woo-buttons' ) ) . ' %s" %s%s>%s</%s>',
					$tag,
